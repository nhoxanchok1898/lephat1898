...
CMD ["sh", "-c", "export DJANGO_SETTINGS_MODULE=paint_store.settings_production && python manage.py migrate --noinput && exec gunicorn paint_store.wsgi:application --bind 0.0.0.0:8000 --workers 3"]
